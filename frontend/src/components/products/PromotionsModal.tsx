          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[58vh] overflow-y-auto pr-1">
            {filteredPromos.map((promo) => {
              ...
              return (
                <div
                  key={promo.id}
                  className={`bg-white rounded-2xl border p-3.5 space-y-2.5 transition shadow-xs ${
                    promo.is_active ? 'border-slate-200 hover:border-indigo-300' : 'border-slate-200/60 opacity-60 bg-slate-50/50'
                  }`}
                >
                  {/* Card Top: Type Badge & Active Switch */}
