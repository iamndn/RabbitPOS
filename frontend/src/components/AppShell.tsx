      ...
      <header className="sticky top-0 z-30 flex items-center justify-between px-4 py-2.5 bg-indigo-600 text-white shadow-md pt-safe">
        ...
        <div className="relative" ref={userMenuRef}>
          <button
            ...
            className="flex items-center space-x-2.5 bg-indigo-700/70 hover:bg-indigo-700 border border-indigo-500/40 py-1.5 px-3 rounded-xl transition cursor-pointer text-left focus:outline-none focus:ring-2 focus:ring-white/40"
            aria-expanded={isUserMenuOpen}
          >
            <div className="w-7 h-7 rounded-lg bg-indigo-500/40 text-indigo-100 font-extrabold flex items-center justify-center text-xs uppercase border border-indigo-400/30">
            ...
            <div className="hidden sm:flex flex-col">
              ...
              <span className="text-[10px] text-indigo-200 uppercase font-semibold leading-tight mt-0.5">
                {currentUser?.role === 'admin' ? t('common.role_admin') : t('common.role_staff')}
              </span>
            </div>
            <ChevronDown className={`w-3.5 h-3.5 text-indigo-200 transition-transform duration-200 ${isUserMenuOpen ? 'rotate-180' : ''}`} />
          </button>

          {/* Dropdown Menu Modal */}
          {isUserMenuOpen && (
            <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 z-50 text-slate-800 animate-in fade-in zoom-in-95 duration-150">
              {/* Header: User Profile Info */}
              <div className="px-4 py-3 border-b border-slate-100 flex items-center space-x-3">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 text-white font-black flex items-center justify-center text-sm shadow-md shadow-indigo-100">
              ...
              <div className="py-1">
                ...
                {currentUser?.role === 'admin' && (
                  <Link
                    ...
                    className="flex items-center space-x-3 px-4 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 hover:text-indigo-600 transition"
                  >
              ...
              <div className="px-1.5 py-0.5">
                <button
                  ...
                  className="w-full flex items-center space-x-3 px-3 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 rounded-xl transition"
                >
